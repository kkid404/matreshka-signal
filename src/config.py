...
from models import LadderStep


# ---------------------------------------------------------------------------
# Top-level scanner config
# ---------------------------------------------------------------------------

@dataclass
class SymbolFilter:
    min_volume_24h: float = 10_000_000  # 10M USDT — filter low-liquidity pairs
...
class TouchConfig:
    ...
    tolerance_value: float = 0.3          # percent
...
class TriggerConfig:
    ...
    min_body_size: float = 0.0001         # absolute, filters doji
    close_position_k: float = 0.5         # close >= low + k*range for LONG
...
class ProbabilityConfig:
    ...
    unresolved_counts_as: float = 0.5     # 0.5 = half-win; 0 = loss
...
class TelegramConfig:
    ...
    chat_id: str = ""             # your chat/channel ID


@dataclass
class ScannerConfig:
    symbols_mode: Literal["all", "top_n", "manual"] = "all"
    top_n: int = 100                      # used when symbols_mode = "top_n"
    symbols: List[str] = field(default_factory=lambda: [
        "BTCUSDT", "ETHUSDT", "SOLUSDT",
    ])                                    # used when symbols_mode = "manual"
    ...
    scan_interval_seconds: int = 900      # 15 min (scan of 200+ symbols takes time)
